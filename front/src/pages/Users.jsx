...
const Users = () => {
  ...
  const promoteUser = async (userId, role) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`admin/promoteuser/${userId}`, { role }, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
            ...
            <tbody>
              {users.map(user => (
                <tr key={user._id} className="border-t">
                  <td className="px-4 py-2"> <Link to={`/profile/${user._id}`} className="text-blue-600 hover:underline">{user.name}</Link></td>
